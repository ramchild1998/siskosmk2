...
class Rekap extends OperatorController {
	...
	public function index() {
		...
		$this->data['kas_id'] = $this->rekap_m->get_data_kas();
		$this->data['jenis_ags'] = $this->rekap_m->get_data_angsuran();
		$this->data['suku_bunga'] = $this->rekap_m->get_data_bunga();
		$this->data['biaya'] = $this->rekap_m->get_biaya_adm();

		$this->data['barang_id'] = $this->rekap_m->get_id_barang();

		$this->data['isi'] = $this->load->view('rekap_list_v', $this->data, TRUE);
	...
	function ajax_list() {
		...
		$data   = $this->rekap_m->get_data_transaksi_ajax($offset,$limit,$search,$sort,$order);
		$i	= 0;
		$rows   = array(); 
		foreach ($data['data'] as $r) {
			...
			$barang = $this->rekap_m->get_data_barang($r->barang_id);
			...
			$rows[$i]['anggota_id_txt'] ='' . sprintf('%04d', $r->anggota_id).'  - '.$anggota->nama.' <br>'.$anggota->departement;
			//$rows[$i]['anggota_id_txt_1'] = ' <br>'.$anggota->nama.' <br>'.$anggota->departement;
			//$rows[$i]['barang_id'] = $r->barang_id;
			//$rows[$i]['lama_angsuran'] = $r->lama_angsuran;

			$rows[$i]['tot_tagihan'] =number_format(nsi_round($total_tagihan));
                        $rows[$i]['jumlah_angsuran'] =  number_format(nsi_round($r->ags_per_bulan)) ;
                        $rows[$i]['sisa_tgh'] = number_format(nsi_round($sisa_tagihan));
                        $rows[$i]['ke_angs'] =' '. $r->bln_sudah_angsur.' / '.$r->lama_angsuran;
                        $rows[$i]['sisa_angs'] = number_format(nsi_round($sisa_angsur)).' - Bulan';
		...
		//		<a href="'.site_url('cetak_pinjaman').'/cetak/' . $r->id . '"  title="Cetak Bukti Transaksi" target="_blank"> <i class="glyphicon glyphicon-print"></i> Nota </a>';
                	...
                	$rows[$i]['nota'] = '<a href="'.site_url('cetak_rekap').'/cetak/' . $r->id . '"  title="Cetak Bukti Transaksi" target="_blank"> <i class="glyphicon glyphicon-print"></i> Nota </a>';
	...
	function get_jenis_barang() {
		...
		$jenis_barang = $this->rekap_m->get_id_barang();
